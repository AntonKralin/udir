...
<html>
	...
	<head>
		<title>«Учет доступа к информационным ресурсам» | Сводная таблица</title>
				...
				<div id="work_all">
					...
					<?php 
						//if (isset($BD_submit)){
							...
							$table_query = "SELECT `table_user`.`fio`, `table_user`.`ip`, `table_base`.`shot_name`, `table_user`.`comp_name`, `table_user`.`ad_login`, `table_user`.`telefon`, `table_user`.`unit`, `table_user`.`job`, `table_user`.`del`, `table_fild`.`id_user`, `table_fild`.`id`, `table_fild`.`request`, `table_fild`.`id_base`, `table_fild`.`login`, `table_fild`.`date_from`, `table_fild`.`date_to`, `table_fild`.`state`, `table_fild`.`date_next`, `table_fild`.`upload`, `table_imns`.`number`, `table_fild`.`notice` from `table_user`, `table_imns`, `table_fild`, `table_base` WHERE `table_fild`.`id_user`=`table_user`.`id` and `table_fild`.`id_base` = `table_base`.`id` and `table_fild`.`id_mns`=`table_imns`.`id` ".$all_mns.$us." ORDER BY `fio`";
