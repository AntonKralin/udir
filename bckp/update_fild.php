			...
			if ( (isset ($_POST['select_base'])) or (isset ($_POST['id_fild']))){
				...
				foreach($fild as $elem){
					...
					foreach($request_state as $buf){
						...
						switch ($buf){
							case "продление":
								if ( ($elem["id_base"]==6) or ($elem["id_base"]==7) or ($elem["id_base"]==9) or ($elem["id_base"]==10) or ($elem["id_base"]==11) or ($elem["id_base"]==12) or ($elem["id_base"]==13) or ($elem["id_base"]==14) or ($elem["id_base"]==15) or ($elem["id_base"]==16) or ($elem["id_base"]==17) or ($elem["id_base"]==24))
									$disable = "disabled";
								break;
							case "разблокировка":
								if ( ($elem["id_base"]==14) or ($elem["id_base"]==18) or ($elem["id_base"]==19) or ($elem["id_base"]==20) or ($elem["id_base"]==24))
									$disable = "disabled";
								break;
							case "изменить реквизиты":
								if ( ($elem["id_base"]==6) or ($elem["id_base"]==7) or ($elem["id_base"]==8) or ($elem["id_base"]==9) or ($elem["id_base"]==10) or ($elem["id_base"]==12) or ($elem["id_base"]==18) or ($elem["id_base"]==19) or ($elem["id_base"]==20) or ($elem["id_base"]==24))
									$disable = "disabled";
								break;
						}
