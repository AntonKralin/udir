...
$avtoBase = null;
if (isset($inputs["id_avto_base"])){
    $_SESSION["avto_base"] = $inputs["id_avto_base"];
    $avtoBase = $inputs["id_avto_base"];
}
if (isset($_SESSION["avto_base"])){
    $avtoBase = $_SESSION["avto_base"];
}

$base = null;
$baseDAO = new BaseDAO();
if ($avtoBase != null){
    $base = $baseDAO->getBaseById($bd, $avtoBase);
}else{
    exit;
}
...
if (isset($inputs["savedate"])){
    $avtoLineDAO->deleteAvtoLineBase($bd, $base->id);
    ...
    if (isset($inputs["name_termination"])){
        ...
        for($i=0; $i<count($nCheck); $i++){
            $avtoLineDAO->insertAvtoLine($bd, $type, $yourname, $nCheck[$i], $dCheck[$i], $base->id);
        }
    }
...
<html>
	...
	<body>
            <form id="form_auto" name="form_auto" autocomplete="off" style="text-align: center" method="post" action="config_avto.php">
                
                <div class="avto_div">
		<?php 
                    echo '<H2>'.$base->shot_name.'</H2>';
                    $line = $avtoLineDAO->getAvtoLineByBase($bd, $base->id);
                ?>
                </div>
				<br>
                <div class="avto_div">
                    ...
                    <?php 
                        ...
                        foreach ($request_state as $elstate){
                            $str = "";
                            if ( $elstate == $type){ 
                ...
                <div class="avto_div">
                    ...
                    for($i=$pos; $i<count($line); $i++ ){
                        ...
                        $yourname=$elem->yourname;
                        echo "<div>";
                        echo "<input type='text' name='name_unlocking[]' placeholder='Наименование' title='Наименование' value='".$elem->name."' />";
                        echo "<input type='text' name='date_unlocking[]' placeholder='Данные' title='Данные' style='width:400px' maxlength=100 value='".$elem->date."' />";
                        echo "<button type='button' onclick='removeElement(this)'>-</button>";
                        echo "</div>";
                    }
                    
                    ?>
                    <button type="button" name="unlocking" onclick="addLine(this);">+</button>
                    <br>
                    <?php 
                        echo "<select name='type_unlocking' onchange='changeType(this,\"yourname_unlocking\")'>";
                        foreach ($request_state as $elstate){
                            $str = "";
                            if ( $elstate == $type){ 
                                    $str='selected="selected"';
                        ...
                        echo "<input type='text' id='yourname_unlocking' name='yourname_unlocking' placeholder='Название таблицы' title='Название таблицы' value='".$yourname."' />";
                    ?>
                </div>
                <br>
                <div class="avto_div">
                    <label>Блок5</label><br>
                    <?php 
                    $type="";
                    $yourname="";
                    for($i=$pos; $i<count($line); $i++ ){
                        ...
                        $yourname=$elem->yourname;
                        echo "<div>";
                        echo "<input type='text' name='name_change[]' placeholder='Наименование' title='Наименование' value='".$elem->name."' />";
                        echo "<input type='text' name='date_change[]' placeholder='Данные' title='Данные' style='width:400px' maxlength=100 value='".$elem->date."' />";
                        echo "<button type='button' onclick='removeElement(this)'>-</button>";
                        echo "</div>";
                    ...
                    ?>
                    <button type="button" name="change" onclick="addLine(this);">+</button>
                    <br>
                    <?php 
                        echo "<select name='type_change' onchange='changeType(this,\"yourname_change\")'>";
                        foreach ($request_state as $elstate){
                            $str = "";
                            if ( $elstate == $type){ 
                                    $str='selected="selected"';
                        ...
                        echo "<input type='text' id='yourname_change' name='yourname_change' placeholder='Название таблицы' title='Название таблицы' value='".$yourname."' />";
                    ?>
                </div>
                <br>
