...
<html>
	...
	<head>
		<title>«Учет доступа к информационным ресурсам» | Отчет по отправлен в мнс</title>
				...
				<div id="work_all">
					...
					<tbody class="table_body">	
						...
						<?php 
							...
							$otchet_query = "select `table_user`.`fio`, `table_user`.`ip`, `table_base`.`shot_name`,  `table_user`.`job`, `table_fild`.`login`, `table_fild`.`date_from`, `table_fild`.`date_to`, `table_fild`.`upload`, `table_fild`.`request`, `table_imns`.`number`, `table_imns`.`name` from `table_user`, `table_imns`, `table_fild`, `table_base` where `table_fild`.`id_user` = `table_user`.`id` and `table_fild`.`id_mns` = `table_imns`.`id` ".$all_mns." and `table_fild`.`id_base` = `table_base`.`id` AND `table_fild`.`state`='".$fild_state[1]."'";
